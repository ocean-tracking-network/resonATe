...
class CohortTest(unittest.TestCase):
    def test_cohort(self):
        ...
        input_df = pd.read_csv('tests/assertion_files/nsbs.csv')
        compressed = compress_detections(input_df, **detect(input_df))
        dfa = cohort(compressed, 3600, **detect(compressed))
