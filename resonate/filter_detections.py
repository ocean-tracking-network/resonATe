...
def filter_detections(detections, suspect_file=None,
    ...
    if suspect_file:
        ...
        user_int = timedelta(minutes=min_time_buffer)
        ...
        raise GenericException("Missing required input columns: {}".format(
            mandatory_columns - set(df.columns)))
    ...
    if distance_matrix:
        ...
        if dm_mandatory_columns.issubset(df.columns):
            ...
            raise GenericException("Missing required input columns for \
                ...
                dm_mandatory_columns - set(df.columns)))
...
def distance_filter(detections, maximum_distance=100000):
    ...
    filtered_detections['filtered'] = distance_df[(distance_df.lag_distance_km <= maximum_distance) & (
        distance_df.lead_distance_km <= maximum_distance)].reset_index(drop=True)
    filtered_detections['suspect'] = distance_df[(distance_df.lag_distance_km > maximum_distance) | (
        distance_df.lead_distance_km > maximum_distance)].reset_index(drop=True)
