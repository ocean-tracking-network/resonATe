...
def cohort(compressed_df, interval_time=60):
    ...
    # Loop through rows in the compressed data file and choose cohorts if the times are within range
    seen = []
    final_set = []

    for idx1, item1 in cmps.iterrows():
        match = cmps[(cmps.station == item1.station) & (cmps.catalognumber != item1.catalognumber) &
        ...
        if not match.empty:
            ...
            final_set.extend(match[['anml_1','anml_1_seq','station','catalognumber',
                                    'seq_num','startdate','enddate',
                                    'startunqdetecid','endunqdetecid','total_count']].values.tolist())
